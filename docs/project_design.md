...
| `main/` | `Kconfig.projbuild` | Display type 说明（当前占位，实际由 `main.h` 定义 `CONFIG_DISPLAY_*` 宏切换） |
...
| `tests/` | `snake.c/h` | LCD 贪吃蛇（16×18 网格，绿身红食物，摇杆方向，200ms/步） |
    ...
    └── CONFIG_DISPLAY_LCD  → lcd_st7735_*() 调用
        （双屏模式时两条路径同时编译）
         ...
         └── [LCD 路径] drivers/lcd_st7735.c
              └── SPI (MOSI=23, SCK=18, CS=5, DC=21, RST=22) @ 26MHz
                  └── ST7735 LCD 128x160
```
...
| 属性 | 值 |
|------|-----|
| 菜单项 | Circle / Button / Display / Snake / About |
| 焦点指示器 | `>` 字符 + 行首空格 |
...
| 校准入口 | 菜单模式下长按中心按钮 >500ms 直接进入 |

### 5.11 关于页面

**单屏模式（OLED-only / LCD-only）**
```
== ESP32 OLED ==          (OLED-only)
== ESP32 LCD ==           (LCD-only)
SSD1306 128x64            (OLED)
ST7735 128x160            (LCD)
Joystick + Menu
...
| 屏幕 | 标题行 | 内容行数 | 一次可见 | 滚动 |
|------|--------|---------|---------|------|
| OLED | `== ESP32 OLED ==` | 8 | `OLED_CONTENT_H` (6) | 需 UP/DOWN 滚动 |
| LCD | `== ESP32 LCD ==` | 9 | `LCD_CONTENT_H` (18) | 无需滚动（全可见） |

双屏时 `about_draw()` 使用 `ssd1306_draw_string()` 和 `lcd_draw_string()` 直接向各自 framebuf 写入不同文本；`display_clear()` / `display_update()` 统一刷清和提交。`scroll_y` 仅控制 OLED 视图偏移，LCD 始终从第 0 行开始绘制。

| 配置 | ABOUT_LINES | ABOUT_VISIBLE | 滚动 |
|------|------------|--------------|------|
| OLED-only | 8 | `CONTENT_H` (6) | 需 UP/DOWN 滚动（8行 > 6行） |
| LCD-only | 10 | `CONTENT_H` (18) | 无需（全可见） |
| Dual (OLED) | `ABOUT_LINES_OLED` = 8 | `OLED_CONTENT_H` (6) | 需 UP/DOWN 滚动 |
| Dual (LCD) | `ABOUT_LINES_LCD` = 9 | —（直接绘满） | 无需（全可见） |

起始 X = 8px。版本行格式 `"Firmware " FW_VERSION_STR`。重绘仅在 `scroll_y` 变化时触发。
...
仅 LCD 模式可用（需要 RGB565 色彩）。白底绿蛇红食物。

| 属性 | 值 |
|------|-----|
| 网格 | 16 列 × 18 行，8×8 px/格 |
| 蛇身颜色 | `COLOR_GREEN` (0x07E0) |
| 食物颜色 | `COLOR_RED` (0xF800) |
| 移动间隔 | 200ms/步 |
| 初始位置 | (0,9)→(1,9)→(2,9)，方向右 |
| 食物放置 | `rand()` 随机，不与蛇重叠 |
| 转向 | 摇杆方向，不能 180° 掉头 |
| 碰撞 | 撞墙或撞自身 → Game Over |
| 重开 | Game Over 后按中心按钮重新开始 |
| 退出 | 长按中心按钮 >500ms 回主菜单（框架内置） |
...
- 5 个菜单项：Circle、Button、Display、Snake、About（校准仅通过菜单模式下长按中心按钮 >500ms 进入，不在菜单列表中）
- 最多显示 3 项（`MENU_VISIBLE = 3`），Snake 为第 4 项，需向下滚动出现
...
| LCD ST7735 | SPI2 | MOSI=23, SCK=18, CS=5, DC=21, RST=22 |
